...
import { Dealer, Message } from "./JSMQ";
...
class IFWorkerCore {
  ...
  async request(target, functionName, args, kwargs) {
    ...
    let contentBuffer = buffer.Buffer.from(msgpack.serialize(content));
    let messageID = "" + this.messageIDs++;
    let message = this._buildMessage(
      messageID,
  ...
  async _onRequest(content, mid, sourcePoint) {
    ...
    try {
      if (func) {
        response["Result"] = await Promise.resolve(
          func.apply(this.serviceObject, args)
        );
      } else {
  ...
  async _onMessage() {
    try {
      ...
      if (message.getSize() === 6) {
        ...
        console.log(
          "Invalid message that contains " + message.length + " frames."
        );
      }
    } catch (e) {
      console.log(e);
  ...
  async _heartbeatLoop() {
    ...
    while (this.running) {
      try {
        this.hbDelay.value = await doHeartbeat(this);
      } catch (error) {
  ...
  _createProxy(path) {
    if (this[path.slice(1)] != undefined) {
